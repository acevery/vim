...
if version < 600
  syntax clear
elseif exists("b:current_syntax") && b:current_syntax =~ "jquery"
  finish
endif
...
syn match   jQuery          /jQuery\(\s*(\)\@=\|\$\(\s*(\)\@=/


syn match   jFunc           /\.\w\+(\@=/ contains=@jFunctions

" for correct highlight in the improved javascript syntax, we need add
"   highlights into @javaScriptAll
syntax cluster javaScriptAll       add=jQuery,@jFunctions
...
syn keyword jEffects        contained hide show toggle effect
...
syn region  javaScriptStringD          start=+"\(\s*<\)\@!+  skip=+\\\\\|\\"+  end=+"\|$+  contains=javaScriptSpecial,@htmlPreproc,@jSelectors
syn region  javaScriptStringS          start=+'\(\s*<\)\@!+  skip=+\\\\\|\\'+  end=+'\|$+  contains=javaScriptSpecial,@htmlPreproc,@jSelectors
